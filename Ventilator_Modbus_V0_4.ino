...
// LCD 
 #define dEnLCD       0    // 使能LCD     0:不使用LCD  1:使用LCD
...
// data array for modbus network sharing
  uint16_t md[16] = {
    15, 20, 25, 0, 0, 0, 0, 8, 0, 0, 0, 0, 0, 0, 1, -1 };
...
void loop() {  
  ...
  if (digitalRead(runPin)==0) {
    moto_action(1,md[mBPM],md[mVOL],md[mSPD]);              //馬達控制RUN
  }else {
    moto_action(0,md[mBPM],md[mVOL],md[mSPD]);               //馬達控制STOP
  }
  showdelay ++ ;
  if (showdelay > 500){
    showdelay = 0 ;
    if (dEnLCD == 1) {
     lcdDisp(1,md[mBPM],md[mVOL],md[mSPD]);
    } 
  }  
}
